...
Per-case F1 ranges from 0.54–0.77. Entity accuracy is 100% across every
case (the fuzzy-vs-strict entity resolver never actually diverged on this
dataset); the real bottleneck is event recall (62–100%) and action-tag
accuracy (40–100%).
...
- **`CASE_ATM_004`'s low action-tag-accuracy score is not an action-tagging
  bug** — its tags are correct for their own content. It's caused by entity
  resolution merging two distinct ground-truth suspects into one pipeline
  entity, which corrupts the evaluator's event-matching (GT events get
  paired to the wrong person's pipeline event). This is the actual next
  lever for improving these scores, not `extract_action_tags()`.
- **Entity resolution is not fully deterministic run-to-run** — whether the
  Groq LLM path or the heuristic fallback fires (depends on API
  availability/rate limits) can change clustering decisions between runs of
  the *same* input, which shows up as recall/precision drift when
  re-evaluating.
